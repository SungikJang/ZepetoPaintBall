...
export default class MyPlayerMovement extends ZepetoScriptBehaviour {
    ...
    if (Manager.Game.IsGamePlaying) {
      ...
      let cq =
        ZepetoPlayers.instance.LocalPlayer.zepetoCamera.camera.gameObject.transform.rotation
          .eulerAngles;
      this.myPlayer.character.gameObject.transform.rotation = Quaternion.Euler(
        new Vector3(q.x, cq.y, q.z),
      );
  ...
  Move(x: float, y: float, amount: float) {
    ...
    this.myPlayer.character.gameObject.transform.position = new Vector3(
      p.x + dir.x * speed,
      p.y,
      p.z + dir.z * speed,
    );
