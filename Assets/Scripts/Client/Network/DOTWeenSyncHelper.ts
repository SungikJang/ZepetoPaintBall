...
export default class DOTWeenSyncHelper extends ZepetoScriptBehaviour {
  ...
  @Tooltip('Calibrates server time delay (default : true)')
  @SerializeField()
  private SyncExtrapolation: boolean = true;
  ...
  @Tooltip('Receives the location again every certain time (default : false)')
  @SerializeField()
  ...
  private CalculateExtrapolation(getPos: Vector3, latency: number) {
    ...
    while (extraOffsetSize > posibleMoveSize) {
      ...
      extraOffSet =
        Vector3.Normalize(this.TweenPosition[this._nextIndex] - getPos) * extraOffsetSize;
  ...
  private *ForceReTargetCoroutine() {
    while (true) {
      if (this.forceReTarget) yield new WaitForSeconds(this.forceReTargetBySeconds);
      else return;
      this.SendPoint();
    }
  }
