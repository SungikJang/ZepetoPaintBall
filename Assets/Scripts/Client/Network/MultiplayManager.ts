...
export default class MultiplayManager extends ZepetoScriptBehaviour {
  ...
  @SerializeField()
  private _pingCheckCount: number = 0;
  ...
  private GetInstantiate() {
    ...
    this.room.AddMessageHandler(MESSAGE.Instantiate, (message: InstantiateObj) => {
      ...
      const spawnPosition = message.spawnPosition
        ? new Vector3(message.spawnPosition.x, message.spawnPosition.y, message.spawnPosition.z)
        : prefabObj.transform.position;
      const spawnRotation = message.spawnRotation
        ? new Quaternion(
            message.spawnRotation.x,
            message.spawnRotation.y,
            message.spawnRotation.z,
            message.spawnRotation.w,
          )
        : prefabObj.transform.rotation;

      const newObj: GameObject = Object.Instantiate(
        prefabObj,
        spawnPosition,
        spawnRotation,
      ) as GameObject;
  ...
  private *WaitPingCheck() {
    if (this.pingCheckCount == 0) yield new WaitUntil(() => this.pingCheckCount > 0);
