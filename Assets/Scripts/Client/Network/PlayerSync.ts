...
export default class PlayerSync extends ZepetoScriptBehaviour {
  ...
  private OnChangedPlayer() {
    ...
    //sync gesture
    if (
      animationParam.State == CharacterState.Gesture &&
      ...
      if (
        ...
        ZepetoPlayersManager.instance.GestureAPIContents.has(gestureName)
      ) {
    ...
    if (this.isUseInjectSpeed) {
      ...
      if (
        animationParam.State == CharacterState.Jump &&
        animationParam.JumpState == CharacterJumpState.JumpIdle
      ) {
        xzSpeed = 0;
  ...
  private *WaitTeleportFrame(waitFrame: number) {
    for (let i = 0; i < waitFrame; i++) yield new WaitForEndOfFrame();
  ...
  private *SendLocalPlayer(tick: number) {
    ...
    while (true) {
      ...
      if (state != CharacterState.Idle || pastIdleCount < pastIdleCountMax) {
        ...
        data.Add(
          'gestureName',
          this._animator.runtimeAnimatorController.animationClips[1].name ?? null,
        );

        const playerAdditionalValue = new RoomData();
        playerAdditionalValue.Add(
          'additionalWalkSpeed',
