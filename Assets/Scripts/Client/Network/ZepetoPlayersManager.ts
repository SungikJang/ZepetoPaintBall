...
import TransformSyncHelper, {
  PositionExtrapolationType,
  PositionInterpolationType,
} from './TransformSyncHelper';
...
export default class ZepetoPlayersManager extends ZepetoScriptBehaviour {
  /** Options **/
  @Header('SpawnOption')
  public readonly ZepetoPlayerSpawnType: ZepetoPlayerSpawnType =
  ...
  @SerializeField()
  private readonly HardSnapIfDistanceGreaterThan: number = 10;
  public readonly InterpolationType: PositionInterpolationType =
    PositionInterpolationType.MoveToward;
  ...
  @SerializeField()
  private readonly extraMultipler: number = 1;
  @Header('Gesture Sync')
  ...
  public static get instance(): ZepetoPlayersManager {
    if (this.m_instance === null) {
      ...
      if (this.m_instance === null) {
        this.m_instance = new GameObject(
          ZepetoPlayersManager.name,
        ).AddComponent<ZepetoPlayersManager>();
      }
    }
      ...
      case ZepetoPlayerSpawnType.MultiplayerSpawnLater:
        ...
        ZepetoPlayers.instance.OnAddedPlayer.AddListener((sessionId: string) => {
          ...
          if (player.isLocalPlayer) {
            ...
            Manager.Game.Players.set(sessionId, player);
            Utils.ChangeLayer(player.character.gameObject, 'outGamePlayer');
            player.character.gameObject.AddComponent<OtherZepetoCharacterController>();
            player.character.gameObject.GetComponent<OtherZepetoCharacterController>().SessionId =
              sessionId;
            //수정
          }
  ...
  private OnJoinPlayer(sessionId: string, player: Player) {
    ...
    if (this.ZepetoPlayerSpawnType == ZepetoPlayerSpawnType.MultiplayerSpawnOnJoinRoom) {
      ...
      spawnInfo.rotation = new Quaternion(0, 0, 0, 0);
      // const homeSpawnPoint = GameObject.Find("HomeSpawnPoint");
      // spawnInfo.position = homeSpawnPoint.transform.position;
      // spawnInfo.rotation = homeSpawnPoint.transform.rotation;
  ...
  private OnLeavePlayer(sessionId: string, player: Player) {
    ...
    Manager.Game.Players.delete(sessionId);
  ...
  private AddPlayerSync(sessionId: string) {
    ...
    const tfHelper =
      zepetoPlayer.character.transform.gameObject.AddComponent<TransformSyncHelper>();
    ...
    const isUseInjectSpeed: boolean =
      this.InterpolationType == PositionInterpolationType.MoveToward ||
