...
import MultiplayManager, {GameObjectStatus} from './MultiplayManager';
...
export default class TransformSyncHelper extends ZepetoScriptBehaviour {
  ...
  @Tooltip('Force the position to be modified if it is farther than this number.')
  public HardSnapIfDistanceGreaterThan: number = 10;
  ...
  @Tooltip('The creditworthiness of the offset figure of the extrapolation.')
  public extraMultipler: number = 1;
  @Tooltip('This is the given speed for lerp / movetoward / fixedspeed options.')
  ...
  private FixedUpdate() {
    if (!this.newGet()) return;
    if (this.isOwner) return;
    if (this._objectStatus != GameObjectStatus.Enable) return;
  ...
  private *BindingState() {
    if (null == this._syncTransform) {
      this._syncTransform = MultiplayManager.instance?.room?.State?.SyncTransforms?.get_Item(
        this._Id,
      );
      if (this._syncTransform) {
        ...
        yield new WaitUntil(
          () => MultiplayManager.instance?.room?.State?.SyncTransforms?.get_Item(this._Id) !== null,
        );
        this.StartCoroutine(this.BindingState());
      }
    }
  ...
  public ChangeOwner(ownerSessionId: string) {
    ...
    if (null == this._room) this._room = MultiplayManager.instance.room;
    if (this._room.SessionId == ownerSessionId && !this._isOwner) {
      ...
      this._isOwner = false;
      if (null != this._sendCoroutine) this.StopCoroutine(this._sendCoroutine);
    }
  }
  ...
  private ChangeStatus(status: GameObjectStatus) {
    if (status == GameObjectStatus.Destroyed) Object.Destroy(this.gameObject);
  }

  public ForceTarget() {
    if (this._isOwner) return;
    if (null == this.newGet()) return;
    if (this.SyncPosition) this.transform.position = this.newGet().position;
    if (this.SyncRotation) this.transform.rotation = this.newGet().rotation;
    if (this.SyncScale) this.transform.localScale = this.newGet().scale;
  }
  ...
  private SyncPositionUpdate() {
    ...
    if (this.UseHardSnap) {
      if (
        Vector3.Distance(this.newGet().position, this.transform.position) >
    ...
    switch (+this.InterpolationType) {
      ...
      case PositionInterpolationType.Lerp:
        this.transform.position = Vector3.Lerp(
          ...
          targetPos,
          extraSpeed * Time.fixedDeltaTime,
        );
        break;
    ...
    switch (+this.ScaleInterpolationType) {
      ...
      case ScaleInterpolationType.Lerp:
        this.transform.localScale = Vector3.Lerp(
          ...
          this.newGet().scale,
          this.scaleUpSpeed * Time.fixedDeltaTime,
        );
        break;
  ...
  private GetExtraPolationOffset() {
    ...
    const moveDirection: UnityEngine.Vector3 = Vector3.Normalize(
      this.newGet().position - this.prevGet().position,
    );
    ...
    switch (+this.ExtrapolationType) {
      ...
      case PositionExtrapolationType.Estimate:
        const estimatedSpeed =
          Vector3.Distance(this.newGet().position, this.prevGet().position) / this._lastGetTimeOut;
  ...
  //     }
  //
  //     Connector.Instance.PlayerAttackRes = (data: {
  //         SessionId: string;
  //         AttackCount: int;
  //     }) => {
  ...
  //         // }
  //     };
  //     Connector.Instance.PlayerSkillRes = (data: {
  //         SessionId: string;
  //         WeaponNumber: number,
  //         SkillNumber: number
  //     }) => {
  //         if (ZepetoPlayers.instance.GetPlayer(data.SessionId).isLocalPlayer) {
  //             MyZepetoCharacterController.Instance.Skill(data.WeaponNumber, data.SkillNumber);
  //         }
  //         else {
  //             const player = ZepetoPlayers.instance.GetPlayer(data.SessionId);
  //             const OtherZCC = player.character.gameObject.GetComponent<OtherZepetoCharacterController>();
  ...
  //     };
  //
  //     Connector.Instance.TrysetMonsterTargetRes = (params) => {
  //         if (params.result) {
  //             Manager.Game.SetMonsterTarget(params.sessionId, params.MonsterPos, params.MonsterName, params.MonsterNum);
  //         }
  //     };
  //
  //     Connector.Instance.MonsterTargetOffRes = (data: {
  //         sessionId: string, Res: boolean, MonsterPos: string, MonsterName: string, MonsterNum: number
  //     }) => {
  //         if (data.Res) {
  //             Manager.Game.SetMonsterTargetOff(data.MonsterPos, data.MonsterName, data.MonsterNum);
  //         }
  //     };
  //
  //     Connector.Instance.MonsterDeadRes = (data: {
  //         MonsterPos: string, MonsterName: string, MonsterNum: number
  //     }) => {
  ...
  //                     mon = point.transform.GetChild(cnt).GetChild(data.MonsterNum - 1).GetChild(0).gameObject;
  //                     break
  //                 }
  //                 cnt += 1;
  //             }
  ...
  //     };
  //
  //     Connector.Instance.MonsterRespawnRes = (data: {
  //         MonsterPos: string, MonsterName: string, MonsterNum: number
  //     }) => {
  //         let point: GameObject = GameObject.Find(data.MonsterPos + 'Points');
  //         let mon: GameObject = null;
  ...
  //             while (true) {
  //                 if (point.transform.GetChild(cnt).gameObject.name === data.MonsterName) {
  //                     mon = point.transform.GetChild(cnt).GetChild(data.MonsterNum - 1).gameObject;
  //                     break
  //                 }
  //             }
  //         }
  ...
  //     };
  //
  //     Connector.Instance.MonterOnDamageRes = (data: {
  //         MonsterPos: string, MonsterName: string, MonsterNum: number, MonsterHp: number
  //     }) => {
  //         let point: GameObject = GameObject.Find(data.MonsterPos + 'Points');
  //         let mon: GameObject = null;
  //         if (point) {
  ...
  //                         mon.GetComponent<DragonPredatorRobot>().monsterHp = data.MonsterHp;
  //                         break;
  //                 }
  //             }
  //         }
