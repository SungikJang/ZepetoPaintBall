...
export default class UIController extends ZepetoScriptBehaviour {
  ...
  public static get Instance(): UIController {
    if (!UIController._instance) {
      const go = GameObject.Find('ZepetoScreenShotUIController');
      UIController._instance = go.GetComponent<UIController>();
    }
  ...
  public ShowCreateFeedResult(result: Boolean) {
    if (result) {
      ...
      this.StartCoroutine(this.ShowToastMessage('Done'));
    } else {
