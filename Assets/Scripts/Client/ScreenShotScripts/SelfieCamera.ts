...
export default class SelfieCamera extends ZepetoScriptBehaviour {
  ...
  public RotateCamera(x: number, y: number) {
    this.SetCameraRotation(
      this.rotateX + x * this.xMouseSensitivity,
  ...
  private CameraMovement() {
    ...
    this.targetLookAt.rotation = Quaternion.Slerp(
      this.targetLookAt.rotation,
      newRot,
      this.smoothCameraRotation * Time.deltaTime,
    );

    var camDir: Vector3 =
      this.targetLookAt.forward * -1 + this.targetLookAt.right * this.rightOffset;
    ...
    lookPoint =
      lookPoint +
      this.targetLookAt.right * Vector3.Dot(camDir * this.distance, this.targetLookAt.right);

    let subtractionVec = new Vector3(
  ...
  public InitSetting(target: Transform) {
    ...
    this.currentTargetPos = new Vector3(
      this.currentTarget.position.x,
      this.currentTarget.position.y,
      this.currentTarget.position.z,
    );
  ...
  LateUpdate() {
    if (this.currentTarget == null || this.targetLookAt == null) return;
